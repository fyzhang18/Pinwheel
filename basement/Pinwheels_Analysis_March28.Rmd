...
ggplot(boo2,aes(x=TIMECODE,y=pupilavg,color=factor(trialtype.pupil),fill=factor(trialtype.pupil)))+
  ...
  theme(legend.position = "bottom")+
  scale_y_continuous(labels = scales::percent,limits=c(-.1,.1),breaks=seq(-.1,.1,.05))
...
ggplot(boo2,aes(x=TIMECODE,y=pupilavg,color=factor(trialtype.pupil),fill=factor(trialtype.pupil)))+
  ...
  scale_y_continuous(labels = scales::percent,limits=c(-.2,.3),breaks=seq(-.2,.3,.1))
...
cor.test(doo.total$percentcorrect, doo.total$percentincorrect, alternative = "two.sided", method = "pearson")
```
